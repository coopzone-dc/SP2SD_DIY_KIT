...
// 1.17a Unknown
//
// 1.17b - Added support for the four LED version from SP2SD showing the boot device at next reset.
...
{//dcdcdcdc
    int i;
     Serial.print(F("\r\nButton pressed, current partition number "));
     Serial.print(initPartition, DEC);
      
 while(1) {//repeat for ever or until error no partions found.
    
  if(digitalRead(ejectPin) == LOW) {
   delay(50); // debounce
  }
   else
  {  //This code skips through the open partion files and stops at the next opened one
  digitalWrite(partition_led_pins[initPartition], LOW); //Turn off old partion LED
